...
RSpec.describe Superthread::Resources::Base do
  ...
  describe '#compact_params' do
    it 'filters out nil values' do
      result = resource.send(:compact_params, name: 'test', color: nil, size: 'large')
      expect(result).to eq(name: 'test', size: 'large')
    end

    it 'keeps false values' do
      result = resource.send(:compact_params, name: 'test', archived: false)
      expect(result).to eq(name: 'test', archived: false)
    end

    it 'keeps empty strings' do
      result = resource.send(:compact_params, name: '', title: 'test')
      expect(result).to eq(name: '', title: 'test')
    end

    it 'returns empty hash when all values are nil' do
      result = resource.send(:compact_params, name: nil, color: nil)
      expect(result).to eq({})
    end
  end
end
